...
export class StripeProvider implements PaymentProvider {
  ...
  private async onUpdateSubscription(
    ...
    const result = await db
      ...
      .returning({ id: payment.id, userId: payment.userId });
    ...
    if (result.length > 0) {
      ...
      // Check if this is a renewal (subscription status is active and period has updated)
      if (
        ...
        try {
          ...
          console.error('Error awarding subscription renewal points:', error);
        }
      }
    } else {
      console.warn(
